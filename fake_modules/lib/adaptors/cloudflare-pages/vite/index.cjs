...
function cloudflarePagesAdaptor(opts = {}) {
  ...
  return viteAdaptor({
    ...
    async generate({ clientOutDir, basePathname }) {
      const routesJsonPath = (0, import_node_path5.join)(clientOutDir, "_routes.json");
      const hasRoutesJson = import_node_fs4.default.existsSync(routesJsonPath);
      if (!hasRoutesJson && opts.functionRoutes !== false) {
        ...
        await import_node_fs4.default.promises.writeFile(routesJsonPath, JSON.stringify(routesJson, void 0, 2));
      }
    }
  });
}
