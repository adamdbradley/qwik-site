    ...
    async generate({ clientOutDir, serverOutDir, basePathname }) {
      ...
      if (opts.outputConfig !== false) {
        ...
        await import_node_fs4.default.promises.writeFile(
          (0, import_node_path5.join)(vercelOutputDir, "config.json"),
          JSON.stringify(vercelOutputConfig, null, 2)
        );
      }
      const vcConfigPath = (0, import_node_path5.join)(serverOutDir, ".vc-config.json");
      const vcConfig = {
        runtime: "edge",
        entrypoint: opts.vcConfigEntryPoint || "entry.vercel-edge.js",
        envVarsInUse: opts.vcConfigEnvVarsInUse
      };
      await import_node_fs4.default.promises.writeFile(vcConfigPath, JSON.stringify(vcConfig, null, 2));
      const staticDir = (0, import_node_path5.join)(vercelOutputDir, "static");
      if (import_node_fs4.default.existsSync(staticDir)) {
        await import_node_fs4.default.promises.rm(staticDir, { recursive: true });
      }
      await import_node_fs4.default.promises.rename(clientOutDir, staticDir);
    }
  });
}
