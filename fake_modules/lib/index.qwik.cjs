...
const RouterOutlet = /* @__PURE__ */ qwik.componentQrl(
  qwik.inlinedQrl(() => {
  ...
  for (let i = contentModules.length - 1; i >= 0; i--) {
    ...
    if (contentModuleHead) {
      if (typeof contentModuleHead === "function")
        resolveDocumentHead(
          head,
          qwik.withLocale(locale, () => contentModuleHead(headProps))
        );
...
function formDataFromArray(array) {
  ...
  for (const [key, value] of array) formData.append(key, value);
  return formData;
}
...
const QwikCityProvider = /* @__PURE__ */ qwik.componentQrl(
  qwik.inlinedQrl(() => {
    const env = useQwikCityEnv();
    console.log("useQwikCityEnv cjs", env);
    ...
    qwik.useTaskQrl(
      qwik.inlinedQrl(
        async ({ track }) => {
          ...
          const { routes, menus, cacheModules, trailingSlash } = await import(
            "@qwik-city-plan"
          );
          ...
          let clientPageData;
          if (build.isServer) clientPageData = env2.response;
          else {
...
const Html = QwikCity;
const QwikCityMockProvider = /* @__PURE__ */ qwik.componentQrl(
  qwik.inlinedQrl((props) => {
...
const Link = /* @__PURE__ */ qwik.componentQrl(
  qwik.inlinedQrl((props) => {
...
class ServerActionImpl {
  ...
  use() {
    ...
    initialState.execute = qwik.inlinedQrl(
      (input) => {
        const [currentAction2, loc2, state2] = qwik.useLexicalScope();
        let data;
        if (input instanceof SubmitEvent) data = new FormData(input.target);
        else if (input instanceof FormData) data = input;
        else data = formDataFromObject(input);
