    ...
    async generate({ clientOutDir, serverOutDir, basePathname }) {
      ...
      if (opts.outputConfig !== false) {
        ...
        await fs4.promises.writeFile(
          join4(vercelOutputDir, "config.json"),
          JSON.stringify(vercelOutputConfig, null, 2)
        );
      }
      const vcConfigPath = join4(serverOutDir, ".vc-config.json");
      const vcConfig = {
        runtime: "edge",
        entrypoint: opts.vcConfigEntryPoint || "entry.vercel-edge.js",
        envVarsInUse: opts.vcConfigEnvVarsInUse
      };
      await fs4.promises.writeFile(vcConfigPath, JSON.stringify(vcConfig, null, 2));
      const staticDir = join4(vercelOutputDir, "static");
      if (fs4.existsSync(staticDir)) {
        await fs4.promises.rm(staticDir, { recursive: true });
      }
      await fs4.promises.rename(clientOutDir, staticDir);
    }
  });
}
export {
